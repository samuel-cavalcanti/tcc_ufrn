...
use zmq_remote_api::{serde_json, sim, RemoteAPIError, RemoteApiClient, RemoteApiClientParams};

pub struct SimpleSceneWithEasyObstacles {
...
impl SimpleSceneWithEasyObstacles {
    pub fn new() -> SimpleSceneWithEasyObstacles {
        SimpleSceneWithEasyObstacles::connect().unwrap()
    }

    fn connect() -> Result<SimpleSceneWithEasyObstacles, RemoteAPIError> {
        ...
        let coppelia_sim: SimpleSceneWithEasyObstacles = SimpleSceneWithEasyObstacles {
            sim,
            client,
            target,
            accelerometer: acc,
