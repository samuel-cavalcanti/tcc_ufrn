...
impl Coppeliasim {
    ...
    fn connect() -> Result<Coppeliasim, RemoteAPIError> {
        ...
        let mass_block = sim.sim_get_object(String::from("./Accelerometer/mass"), None)?;
        let force_sensor = sim.sim_get_object(String::from("./Accelerometer/forceSensor"), None)?;

        let right_motor = sim.sim_get_object(String::from("./DynamicRightJoint"), None)?;
        let left_motor = sim.sim_get_object(String::from("./DynamicLeftJoint"), None)?;

        let rgb_kinect_handle = sim.sim_get_object(String::from("./kinect/rgb"), None)?;
        let kinect_depth = sim.sim_get_object(String::from("./kinect/depth"), None)?;

        let target_handle = sim.sim_get_object(String::from("./target"), None)?;

        let robot_handle = sim.sim_get_object(String::from("./metade_lateral_r"), None)?;

        let mut target = Target::new(target_handle);
        target.move_target(&sim)?;

        let mass = sim.sim_get_object_float_param(mass_block, sim::SHAPEFLOATPARAM_MASS)?;
    ...
    pub fn enable_step(&self) {
        self.sim.set_stepping(true).unwrap();
    }

    pub fn step(&self) {
        self.sim.step(true).unwrap();
    }

    pub fn get_target_distance(&self) -> Vec<f64> {
        self.sim
            .sim_get_object_position(self.target.handle, self.robot_handle)
            .unwrap()
    }

    pub fn get_target_pos(&self) -> Vec<f64> {
        self.sim
            .sim_get_object_position(self.target.handle, sim::HANDLE_WORLD)
            .unwrap()
    }

    pub fn get_robot_pos(&self) -> Vec<f64> {
        self.sim
            .sim_get_object_position(self.robot_handle, sim::HANDLE_WORLD)
            .unwrap()
    }
    ...
    pub fn get_time(&self) -> f64 {
        self.sim.sim_get_simulation_time().unwrap()
    }
    pub fn get_orientation_z(&self) -> f64 {
        let ori = self
            .sim
            .sim_get_object_orientation(self.robot_handle, sim::HANDLE_WORLD)
            .unwrap();
        ori[2]
    }

    pub fn start_simulation(&self) {
        self.sim.sim_start_simulation().unwrap();
    }

    pub fn stop_simulation(&self) {
        self.sim.sim_stop_simulation().unwrap();
    }

    pub fn set_velocity(&self, left: f64, right: f64) {
        self.sim
            .sim_set_joint_target_velocity(self.motors_handle.left_handle, left as f64, None)
            .unwrap();
        self.sim
            .sim_set_joint_target_velocity(self.motors_handle.right_handle, right as f64, None)
            .unwrap();
    }
}
